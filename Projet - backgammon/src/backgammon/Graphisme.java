...
public class Graphisme extends JFrame implements ActionListener{
	...
	public  String affichage (Colonne [] plateau) 
	{
		...
		for (int i = 0; i < 26; i++) 
		{
			...
			String ligne = plateau[i].toString() + " : " + 
					plateau[i].getCompteur() + " pions " + couleur + plateau[i].colonnesPossibles(plateau, joueurs[(this.couleur - 1)]) + "\n" ;
			retour = retour + ligne;
		}
	...
	public void boutonsAdaptation () 
	{
		...
		if(!joueurs[couleur - 1].isJouable(plateau)) {passageTour();}
		if (c == 0) 
		{
			for (int i = 0; i < 26; i++)
			{
		...
		if(couleur == 1 && plateau[0].getCompteur() > 0) 
		{
			for(int i = 1; i< 26; i++)
			{
				colonnes[i].setEnabled(false);
		...
		if(couleur == 2 && plateau[25].getCompteur() > 0) 
		{
			for(int i = 24; i > -1; i--)
			{
				colonnes[i].setEnabled(false);
